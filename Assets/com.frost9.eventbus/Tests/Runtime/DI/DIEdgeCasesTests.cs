...
namespace Frost9.EventBus.Tests
{
    ...
    public class DIEdgeCasesTests
    {
        ...
        [Test]
        public void EventBus_ConflictingRegistrations_ThrowsException()
        {
            var builder = new ContainerBuilder();

            // Register EventBus manually first
            builder.Register<EventBus>(Lifetime.Singleton).As<IEventBus>();

            // Then try to install (should conflict)
            var installer = new EventBusInstaller();
            installer.Install(builder);

            Assert.Throws<VContainerException>(() => builder.Build(),
                "Conflicting EventBus registrations should throw during container build");
        }
        ...
        [Test]
        public void EventBus_EmptyContainerBuilder_InstallationWorks()
        {
            var builder = new ContainerBuilder();
            var installer = new EventBusInstaller();

            // Install on completely empty builder
            Assert.DoesNotThrow(() => installer.Install(builder), "Should install on empty builder");
        ...
        [Test]
        public void EventBus_NullBuilderInstallation_ThrowsException()
        {
            var installer = new EventBusInstaller();

            Assert.Throws<ArgumentNullException>(() => installer.Install(null),
                "Installing with null builder should throw ArgumentNullException");
        }
